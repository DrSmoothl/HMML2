...
class SetRootPathRequest(BaseModel):
    """API请求 - 设置主程序根目录"""
    main_root: str = Field(..., description="新的麦麦主程序根目录", min_length=1, max_length=500)


class AddAdapterRootRequest(BaseModel):
    """API请求 - 添加适配器根目录"""
    adapter_name: str = Field(..., description="适配器名称", min_length=1, max_length=100)
    root_path: str = Field(..., description="适配器根目录路径", min_length=1, max_length=500)
...
class UpdateAdapterRootRequest(BaseModel):
    """API请求 - 更新适配器根目录"""
    adapter_name: str = Field(..., description="适配器名称", min_length=1, max_length=100)
    root_path: str = Field(..., description="新的根目录路径", min_length=1, max_length=500)
