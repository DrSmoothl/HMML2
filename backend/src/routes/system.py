...
def is_onekey_environment() -> bool:
    ...
    要求目录链: */modules/HMML2Backend/backend/src/routes/system.py
    不再强制要求 modules 下存在 MaiBot 目录（允许后续再放置或裁剪）。
    仅通过层级与关键目录名判定。
    """
    try:
        fp = Path(__file__).resolve()
        routes_dir = fp.parent
        src_dir = routes_dir.parent
        parent_dir = src_dir.parent  # 可能是 backend 或 HMML2Backend
        variant = None

        # 变体 A: modules / HMML2Backend / backend / src / routes
        ...
        # 变体 B: modules / HMML2Backend / src / routes (无 backend 层)
            ...
            logger.info('[OneKeyCheck][system] 未匹配结构: parent_dir=%s parent_parent=%s', parent_dir.name, parent_dir.parent.name if parent_dir.parent else None)
            return False

        logger.info('[OneKeyCheck][system] variant=%s fp=%s', variant, fp)
        logger.info('[OneKeyCheck][system] routes=%s src=%s hmml2=%s modules=%s',
                     routes_dir.name, src_dir.name, hmml2backend_dir.name, modules_dir.name)

        if routes_dir.name != 'routes' or src_dir.name != 'src':
            return False
        ...
        logger.info(f'[OneKeyCheck][system] 检测异常: {e}')
        return False
